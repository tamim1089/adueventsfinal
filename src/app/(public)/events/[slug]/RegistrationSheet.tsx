...
export default function RegistrationSheet({
  ...
  const matches = useMemo(() => {
    ...
    if (schoolId) return [];
    if (!q) return schools.slice(0, 6);
    return schools.filter((s) => s.name.toLowerCase().includes(q)).slice(0, 6);
  }, [schoolQ, schoolId, schools]);

  async function submit(e: React.FormEvent) {
    e.preventDefault();
    if (pending) return;
    setPending(true);
    const res = await registerForEvent({ eventId: event.id, kind, fullName, email, uniId, position, schoolId, grade });
    if (res.ok) { setDone(true); setPending(false); toast.success("You're registered"); }
    else { setPending(false); toast.error(res.error); }
  }
    ...
    <AnimatePresence>
      {open && (
        <>
          ...
          />
          <motion.div
            className="fixed inset-x-0 bottom-0 z-[101] mx-auto flex max-h-[92dvh] w-full max-w-lg flex-col overflow-hidden border border-[var(--glass-border)] bg-[var(--bg-base)] shadow-2xl sm:inset-y-0 sm:my-auto sm:h-fit"
          ...
          >
            ...
            <div className="flex items-start justify-between gap-4 px-6 pt-4 sm:pt-6">
              <div>
                ...
                <h2 dir="auto" className="mt-1 font-display text-2xl font-bold leading-tight text-[var(--text-primary)]">{event.title}</h2>
              </div>
              <button onClick={() => setOpen(false)} aria-label="Close" className="rounded-full p-2 text-[var(--text-tertiary)] hover:bg-[var(--bg-subtle)]"><X size={18} /></button>
            </div>
            ...
            {done ? (
              <div className="px-6 pb-8 pt-6 text-center">
                ...
                <button onClick={() => setOpen(false)} className="mt-6 rounded-full px-6 py-3 text-sm font-semibold text-[var(--accent-on)]" style={{ background: "var(--accent)" }}>Done</button>
              </div>
            ) : (
              <form onSubmit={submit} className="flex-1 overflow-y-auto px-6 pb-8 pt-5">
                ...
                <div className="grid grid-cols-2 gap-2">
                  {([["uni", "ADU student / staff", Building2], ["school", "School student", GraduationCap]] as const).map(([k, lbl, Icon]) => (
                    <button key={k} type="button" onClick={() => setKind(k)}
                      className={`flex items-center gap-2 rounded-xl border px-3.5 py-3 text-left text-sm font-medium transition-colors ${kind === k ? "border-[var(--accent)] text-[var(--text-primary)]" : "border-[var(--glass-border)] text-[var(--text-secondary)] hover:text-[var(--text-primary)]"}`}>
                    ...
                    <>
                      <div>
                        ...
                        <div className="relative">
                          ...
                          <Input className={`${fieldCls} pl-10`} value={schoolQ}
                            onChange={(e) => { setSchoolQ(e.target.value); setSchoolId(""); }}
                            placeholder="Search your school…" autoComplete="off" />
                        </div>
                        {/* inline list — never clipped */}
                        {matches.length > 0 && (
                          <div className="mt-2 max-h-44 overflow-y-auto rounded-xl border border-[var(--glass-border)]">
                            {matches.map((s) => (
                              <button key={s.id} type="button"
                                onClick={() => { setSchoolId(s.id); setSchoolQ(s.name); }}
                                className="block w-full border-b border-[var(--glass-border)] px-4 py-2.5 text-left text-sm text-[var(--text-primary)] last:border-0 hover:bg-[var(--bg-subtle)]">
                                {s.name}{s.category ? <span className="ml-2 text-xs text-[var(--text-tertiary)]">{s.category}</span> : null}
                              </button>
                            ))}
                          </div>
                        )}
                        {schoolId && <p className="mt-1.5 text-xs text-[var(--accent)]">School selected ✓</p>}
                      </div>
                      <div>
                ...
                <p className="mt-3 text-center text-xs text-[var(--text-tertiary)]">Registering counts as attendance. Your certificate is emailed after the event.</p>
              </form>
            )}
          </motion.div>
        </>
  ...
  return (
    <>
      <button type="button" onClick={() => { setDone(false); setOpen(true); }}
